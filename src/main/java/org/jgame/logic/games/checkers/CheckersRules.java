 ...
 * - Regular pieces move forward only, kings move any diagonal direction
 * - Captures are mandatory when available
 * - Multi-jump chains must be completed
 * - Piece promotes to king when reaching opposite end
 ...
 * @version 1.0
 */
public class CheckersRules extends AbstractRuleset {
    ...
    private void checkWinConditions() {
        ...
        if (opponentPieces.isEmpty()) {
            gameOver = true;
            winner = (currentPlayer == 1) ? 2 : 1; // Previous player wins
            return;
        }
